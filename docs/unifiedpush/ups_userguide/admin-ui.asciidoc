...
The Red Hat JBoss Unified Push Server can be accessed via its RESTful endpoints, as well over an administrative user interface.
...
The difference between the +admin+ and +developer+ account types is that the former can access all PushApplications and Variants configured on the server, whereas the latter has access only to the PushApplications and Variants created by the actual user. Further, the administration of the Red Hat JBoss Unified Push Server, as well as management of it's users in the **Keycloak Admin Console**, can only be performed by the +admin+.
...
To go back to the AdminUI click the marked link on the right corner, this performs a redirect to the previous landing page:
...
Now it is time to send a test message to the device using the +Send Push+ feature of the Red Hat JBoss Unified Push Server! For that we select the PushApplication we would like to use:
...
For our supported SDKs the Red Hat JBoss Unified Push Server has a feature to generate code snippets for the backend part as well. In the above screen, click on the +Example implementation+ link to get the code snippets:
...
The Dashboard is a nice way to learn what's going on in the Red Hat JBoss Unified Push Server. It presents a number of PushApplications, Sent Push Notifications and a total number of devices, registered with the Red Hat JBoss Unified Push Server:
...
The metadata is deleted after 30 days, using a nightly job within the Red Hat JBoss Unified Push Server.
...
Here you are also able to see which supported SDK was used to send the message. In this example the message was sent from the Console of the Red Hat JBoss Unified Push Server.
