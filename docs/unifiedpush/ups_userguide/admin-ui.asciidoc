...
Red Hat JBoss Unified Push Server can be accessed via its RESTful endpoints, as well over the administrative user interface.

Admin and Developer accounts
~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Once installed, Red Hat JBoss Unified Push Server comes with two different account types:

* a single +admin+ user
* an initial +developer+ account

The difference between the +admin+ and +developer+ account types is that the former can access all PushApplications and Variants configured on the server, whereas the latter has access only to the PushApplications and Variants created by the actual user. Furthermore, the administration of Red Hat JBoss Unified Push Server, as well as management of its users in the **Keycloak Admin Console**, can only be performed by the +admin+ account.
...
Once you have Red Hat JBoss Unified Push Server running, whether on link:../openshift[Openshift] or link:../server-installation[standalone], it is time to open the _AdminUI_, by accessing +http(s)://SERVER:PORT/ag-push+ in your browser. You are prompted by a login dialog:
...
After applying the new password you are able to use the AdminUI of Red Hat JBoss Unified Push Server. The Dashboard is welcoming you:
...
Once you are at the point where your device is registered with Red Hat JBoss Unified Push Server, refresh the details of the variant and you will notice an updated number of installations:
...
Red Hat JBoss Unified Push Server has a feature to generate code snippets for the backend part as well for our supported SDKs. In the above screen, click on the +Example implementation+ link to get the code snippets:
...
Using of the Dashboard is handy in order to learn what is going on in your Red Hat JBoss Unified Push Server installation. It presents a number of PushApplications, Sent Push Notifications and a total number of devices which are registered with Red Hat JBoss Unified Push Server:
...
The metadata is deleted after 30 days, using a scheduled job within Red Hat JBoss Unified Push Server.
...
Here you are also able to see which supported SDK was used to send the message. In this particular example, the message was sent from the Console of Red Hat JBoss Unified Push Server.
